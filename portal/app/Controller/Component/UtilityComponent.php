...
class UtilityComponent extends Component {
    ...
    public function postEmail($api, $type, $params) {
        ...
        //VIRIFICAMOS SE USUÁRIO SELECIONOU A OPÇAO DE RECEBER EMAILS OS NOTIFACAÇOES
        $sqlUsing = "select * from users_using where id = ";
    ...
    public function postSendNotification($params) {

        $sqlPeriod = "select * from users_preferences inner join users_using on users_using.user_id = users_preferences.user_id "
                . "where users_preferences.user_id = {$params['user_id']};";
    ...
    public function urlRequestToSendNotification($params) {
        ...
        $retorno = $this->curlRequest("http://54.207.82.125/stores/docemel/parse-docemel/index.php", $params);
        return $retorno;
    }
    
    public function urlRequestToSendEmailPost($api, $type, $params) {
        $apiUrl = API_ENVIRONMENT_URL . '/';
        $apiUrl .= $api . '/postEmailPost';
        $apiUrl .= '/' . $type;
