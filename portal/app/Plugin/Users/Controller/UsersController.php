...
//                    $this->Email->send($mensagem);
    ...
    public function testa($page, $compId) {
		...
		$us = $this->Session->read('userData.User');
       if (!empty($us)) {
            $this->usersNews();
        } 
		}
    ...
    public function wish($page) {
        ...
        if ($page == 'save') {
            ...
            $par = array('Company' => array('fields' => array('id'), 'conditions' => array('category_id' => $param['UsersWishlist']['category_id'], 'sub_category_id' => 5, 'status' => 'ACTIVE')));
...
//
    ...
    public function ajaxLogin($page) {
        ...
        if ($page == 'createUser') {
            ...
            $sqlPref = "insert into users_preferences(user_id, notifications_periodicity, notifications_type) values({$user['data']['User']['id']}, 'UNITARY', 'EMAIL');";
            $paramsPref = array(
                'User' => array(
                    'query' => $sqlPref
                )
			...
			//ARRAY COM INFOS DO EMAIL A SER ENVIADO
			//ENVIA EMAIL DE BOAS VINDAS AO USUÁRIO
			$sendEmail = '';
			$sendEmail['user_id'] = $user['data']['User']['id'];
			$sendEmail['email_type'] = 'newUser';
			$sendEmail['api'] = 'users';
			$sendEmail['userName'] = $nome;
			$sendEmail['userEmail'] = $email;
			$sendEmail['pwd'] = $pass;
			
			$this->Utility->postEmail("users", "newUser", $sendEmail);
        ...
        if ($page == 'logar') {
            ...
            $preferenceQuery = "select * from users_preferences where user_id={$usuario['User']['id']};";
...
//        $offerNotification['InvitesToMe'] = $invitesToMe;
    ...
    public function meusDados($page) {
        ...
        if ($page == 'ajax-cep') {
		  ...
		  if ($page == 'save-notification') {
            ...
            if (!empty($select)) {
                $sqlUpdate = "update users_preferences set notifications_periodicity = '{$periodicity}', notifications_type = '{$notificationType}' where user_id = {$id};";
                $paramsUpdate = array('User' => array('query' => $sqlUpdate));
                $this->Utility->urlRequestToGetData('users', 'query', $paramsUpdate);
            } else {
                $sqlNotif = "insert into users_preferences(user_id, notifications_periodicity, notifications_type) values({$id}, '{$periodicity}', '{$notificationType}');";
        ...
        if ($page == 'save-preferences') {
			...
			$usId = $this->Session->read('userPreferences.id');
            if (!empty($usId)) {
        ...
        if ($page == 'save-social-networks') {
			...
			$ussId = $this->Session->read('userSocial.id');
            if (empty($ussId)) {
                //face
    ...
    public function changeLogin() {
		...
		$arrayParams = array('Payments' => array(
            ...
            //$dados = $this->Utility->urlRequestToCheckout('payments', $arrayParams);
			echo "<div style=\"margin-left: 25%;\">".$email.'<br/>'.print_r($dados)."</div>";*/
			
			$param['email'] = '';
			
			//$return = $this->Utility->postEmail('companies', 'newCompany', $param);
			
			//$updateStatus = $this->Utility->urlRequestCompanyTesteEmail('companies','all', $param);
			
			$email = '';
			$email['user_id'] = 311;
			$email['email_type'] = 'newUser';
			$email['api'] = 'users';
			$email['userName'] = 'Matheus Odilon';
			$email['userEmail'] = "matheusodilon0@gmail.com";
			$email['pwd'] = "password";	


			//$envio = $this->Utility->postEmail("companies", "newCompany", $email);
	 ...
	 $param['notifText'] = "Esse é o corpo da notificação";
        ...
        $limite = 8;
        $this->set(compact('limite', 'offers2', 'perfil', 'pageName'));
        $this->render('Elements/ajax_vitrine');
    }
	...
	function cronTeste(){
		...
		$retorno = $this->Utility->postEmailAutomatic('users', 'birthday', $data);
		
		echo "retorno: ". print_r($retorno);
	}
	
	//DESTRÓI A VÁRIAL DE PRIMEIRO USUARIO 
	public function unsetFirstUser() {
